...
OUTPUT_REPORT_PATH = os.path.join(WORKSPACE_ROOT, "outputs", "recurrent_types_500q_comparison_report.json")
...
def main():
    print("=" * 80)
    print(" 500Q BENCHMARK SUITE: RECURRENT DEPTH TYPES VS BASELINE ")
    print(f" Device: {DEVICE} | Batch Size: {BATCH_SIZE}")
    print("=" * 80)

    # 1. Load 500 questions per category
    print("\nLoading Datasets (500 Questions x 5 Categories = 2,500 total) ...")
    categories = {
    ...
    print("\n" + "=" * 115)
    print("  FINAL BENCHMARK COMPARISON: 6 RECURRENT TYPES VS 24-LAYER BASELINE")
    print("=" * 115)
    header = f"{'Model Configuration':<42} | {'Math':<6} | {'Code':<6} | {'Sci':<6} | {'Hist':<6} | {'Logic':<6} | {'Overall':<8} | {'Params':<8} | {'Disk MB':<7} | {'VRAM MB'}"
    print(header)
    print("-" * len(header))
    ...
    for m_name, d in all_results.items():
        ...
        print(f"{m_name[:42]:<42} | {m_acc:>6} | {c_acc:>6} | {s_acc:>6} | {h_acc:>6} | {l_acc:>6} | {tot_acc:>8} | {params_m:>8} | {disk_mb:>7} | {vram_mb:>7}")
